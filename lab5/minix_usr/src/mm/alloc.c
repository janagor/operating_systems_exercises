...
PRIVATE int alloc_alg = WORST_FIT_ALG;
/* JAN_END */
...
{
  if (mm_in.m1_i1 == WORST_FIT_ALG || mm_in.m1_i1 == FIRST_FIT_ALG)
...
PUBLIC int do_hole_map()
{
  ...
  unsigned nbytes, ctr, npairs, buffer[1024];

  ctr = 0;
  nbytes = mm_in.m1_i1;
  if (nbytes) nbytes -= 1 - (nbytes % 2);
  npairs = nbytes >> 1;

  hp = hole_head;
  while (hp != NIL_HOLE && hp->h_base < swap_base && ctr <= 1021) {
  	buffer[ctr++] = hp->h_len;
  	buffer[ctr++] = hp->h_base;
  	hp = hp->h_next;
  }
  if (ctr < nbytes) {
	/* holes number < buffer capacity */
  	npairs = ctr >> 1;
  }
  buffer[2*npairs] = 0;
  sys_copy(
  	0, D, (phys_bytes)buffer,
