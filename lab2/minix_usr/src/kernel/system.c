...
{
  struct proc *pp;
  unsigned group_id = m_ptr->m1_i2;
  if (group_id >= 3) return(EINVAL);
  ...
  for (pp = BEG_PROC_ADDR; pp < END_PROC_ADDR; pp++) {
	...
	if (pp->p_pid == m_ptr->m1_i1) {
		...
		if (group_id == AGING) pp->age = m_ptr->m1_i3;
		if (group_id == SHORTEST_JOB_FIRST) pp->time = m_ptr->m1_i3;
		return(OK);
	}
  }
  return(ESRCH);
...
/* Handle sys_fork().  m_ptr->PROC1 has forked.  The child is m_ptr->PROC2. */
  ...
  *rpc = *rpp;			/* copy 'proc' struct */
  ...
  /* JAN_BEGIN */
  /* assign default value of age to the child process */
  /*rpc->age = 0;*/
  /* JAN_END*/
