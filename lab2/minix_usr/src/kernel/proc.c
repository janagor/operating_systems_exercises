 ...
 */
  ...
  */
  ...
  child = current->p_nextready;


  switch (rp->group_id) {
  case ROUND_ROBIN:
	while(child != NIL_PROC) {
		if (child->group_id > rp->group_id) break;
		current = child;
		child = child->p_nextready;
	}
  	current->p_nextready = rp;
  	rp->p_nextready = child;
	if (child == NIL_PROC) rdy_tail[USER_Q] = rp;
  	break;
  ...
  case AGING:
	...
	if (child == NIL_PROC) rdy_tail[USER_Q] = rp;
  	current->p_nextready = rp;
  	rp->p_nextready = child;
  	current->p_nextready->p_nextready = child;
  ...
  case SHORTEST_JOB_FIRST:
	...
	rp->p_nextready = child;
  	...
  	break;
  default:
  	printf("VERY VERY BAD. DIFFERENT ID THEN ANY OF EXPECTED.\n");
  	printf("CHANGING ITS ID TO ROUND ROBIN");
  }
  if (rp->p_nextready == rdy_head[USER_Q]) rdy_head[USER_Q] = rp;
 ...
 */
  ...
  */
  ...
  switch (rdy_head[USER_Q]->group_id) {
  case ROUND_ROBIN:
	while(child != NIL_PROC) {
		if (child->group_id > rdy_head[USER_Q]->group_id) break;
		current = child;
		child = child->p_nextready;
	} 
	if (child == NIL_PROC) rdy_tail[USER_Q] = rdy_head[USER_Q];
  	current->p_nextready = rdy_head[USER_Q];
  	rdy_head[USER_Q] = rdy_head[USER_Q]->p_nextready;
  	current->p_nextready->p_nextready = child;
  	rdy_tail[USER_Q]->p_nextready = NIL_PROC;
  ...
  case AGING:
	...
	while(child != NIL_PROC) {
		if (child->age < rdy_head[USER_Q]->age) break;
		if (child->group_id > rdy_head[USER_Q]->group_id) break;
		current = child;
		child = child->p_nextready;
	} 
  ...
  case SHORTEST_JOB_FIRST:
	...
	while(child != NIL_PROC) {
		if (child->time > rdy_head[USER_Q]->time) break;
		if (child->group_id > rdy_head[USER_Q]->group_id) break;
		current = child;
		child = child->p_nextready;
	} 
  ...
  default:
  	printf("VERY VERY BAD. DIFFERENT ID THEN ANY OF EXPECTED.\n");
  	printf("CHANGING ITS ID TO ROUND ROBIN");
  	rdy_head[USER_Q]->group_id = ROUND_ROBIN;
